...
class _RemesasState extends State<Remesas> {
  ...
  getAllRemesas() async {
    ...
    if (!_Remesas.isEmpty) {
      ...
      _frecuenciaApoyo.text =
          _Remesas.map((e) => e.frecuencia.toString()).first;
    } else {
      alertDialog(context, 'Sin datos para mostrar');
    }
  }
  ...
  actualizar() async {
    ...
    await DbHelper().upDateRemesas(BModel).then((remesasModel) {
      alertDialog(context, "Se actualizaron correctamente");

...
class DatosGenerales extends StatefulWidget {
  ...
  String folio;
  String folioDisp;
  DatosGenerales({this.usuario, this.folio, this.folioDisp, this.dispositivo});
...
class _DatosGeneralesState extends State<DatosGenerales> {
  ...
  actualizar() async {
    ...
    getComunidades();
    getAllAsentamiento();
    print(_tipoVialidad.text.replaceAll("  ", " "));
    _TiposVialidad2 = [];
    var datos = await CategoryService()
        .readTipoVialidad(_tipoVialidad.text.replaceAll("  ", " "));
    datos.forEach((category) {
      setState(() {
        var data = TiposVialidad();
        data.Orden = category['Orden'];
  ...
  getAllDatosgenerales() async {
    ...
    categories.forEach((category) {
      ...
      inco = _DatosGenerales.map((e) => e.incompleto).first;
    });
  }

  save() async {
    if (_DatosGenerales.isEmpty) {
      insertarDatos();
    } else {
      actualizar();
    }
